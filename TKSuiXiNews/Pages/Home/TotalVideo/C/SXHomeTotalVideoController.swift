...
class SXHomeTotalVideoController: SXBaseViewController {
    //初始化
    var startIndex:Int = 0
    
    //初始化页面选择器
    lazy var _pageViewManager: DNSPageViewManager = {
        ...
        let childViewControllers: [UIViewController] = titles.enumerated().map { (index, _) -> UIViewController in
            //V视频
            var controller: SXHomeTotalChildVideoController!
            controller = SXHomeTotalChildVideoController();
    ...
    private func setupUI() {
        ...
        let contentView = _pageViewManager.contentView
        view.addSubview(_pageViewManager.contentView)
        ...
        // 单独设置titleView的frame
        let titleView = _pageViewManager.titleView
        //设置阴影
        ...
        view.addSubview(_pageViewManager.titleView)
