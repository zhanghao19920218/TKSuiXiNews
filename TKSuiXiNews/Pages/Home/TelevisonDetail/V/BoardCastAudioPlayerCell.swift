...
class BoardCastAudioPlayerCell: BaseTableViewCell {
    ///懒加载播放器
    ...
    var audioPlayerUrl:String? {
        willSet(newValue) {
            //初始化播放器
            player = PLPlayer.init(url: URL(string: newValue ?? ""), option: option)
...
extension BoardCastAudioPlayerCell: PLPlayerDelegate {
