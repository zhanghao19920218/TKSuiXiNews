...
fileprivate let mallProductIdentifier = "MallProductCollectionViewCellIdentifier"

class SXScoreMallController: BaseCollectionViewController {
    ...
    private func _setupUI() {
        ...
        collectionView.register(SXMallProductCollectionViewCell.self, forCellWithReuseIdentifier: mallProductIdentifier)
    ...
    override func pullDownRefreshData() {
        ...
        HttpClient.shareInstance.request(target: BAAPI.scoreItemList(p: page), success:{ [weak self] (json) in
            ...
            let model = try? decoder.decode(SXMallListProductItemResponse.self, from: json)
    ...
    @objc private func scoreButtonTapped(_ sender: UIButton) {
        let vc = SXRotatePanelScoreController()
        navigationController?.pushViewController(vc, animated: true)
    }
    
    @objc private func scoreRuleButtonTapped(_ sender: UIButton) {
        let vc = SXScoreRuleViewController()
        navigationController?.pushViewController(vc, animated: true)
    }
}


extension SXScoreMallController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let model = dataSource[indexPath.row] as! SXMallListProductItemModel
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: mallProductIdentifier, for: indexPath) as! SXMallProductCollectionViewCell
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let model = dataSource[indexPath.row] as! SXMallListProductItemModel
        let vc = SXScoreProductDetailViewController()
        vc.goodId = model.id.int
        navigationController?.pushViewController(vc, animated: true)
    }
...
extension SXScoreMallController {
    //MARK: - 请求首页数据
