//
//  ServiceWKWebViewController.swift
//  TKSuiXiNews
//
...
/// 显示服务的显示的WebView
class SXServiceWKWebViewController: SXBaseViewController {
    //MARK: - 加载网页
    var loadUrl: String? {
        willSet(newValue) {
            if let value = newValue {
                ...
                if let url = URL(string: value) {
                    _webView.load(URLRequest.init(url: url));
                }
            }
        }
    }
...
extension SXServiceWKWebViewController: WKUIDelegate, WKNavigationDelegate {
        ...
        if keyPath == "estimatedProgress" {
            ...
            if _webView.estimatedProgress >= 1.0 {
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) { [weak self] () in
                    self?._progressView.progress = 0; //重新为0
                    self?._progressView.isHidden = true
                }
            }
            
        } else if keyPath == "title"{
            navigationItem.title = _webView.title
        } else {
