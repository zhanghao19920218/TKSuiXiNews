...
///转盘抽奖的Controller
class SXRotatePanelScoreController: SXBaseViewController {
    private var _model: SXAwardPanelModel = SXAwardPanelModel()
    ...
    private lazy var _panelView: SXPanelTurnView = {
        let view = SXPanelTurnView()
...
extension SXRotatePanelScoreController: CAAnimationDelegate {
...
extension SXRotatePanelScoreController {
    private func requestData(){
        HttpClient.shareInstance.request(target: BAAPI.awardProductItems, success: { [weak self] (json) in
            let decoder = JSONDecoder()
            let model = try? decoder.decode(SXRotatePanelListItemResponse.self, from: json)
    ...
    private func requestOnceTime() {
        ...
        HttpClient.shareInstance.request(target: BAAPI.awardDraw, success: { [weak self] (json) in
            let decoder = JSONDecoder()
            let model = try? decoder.decode(SXAwardDrawResponse.self, from: json)
            guard let forceModel = model else {
                return;
            }
